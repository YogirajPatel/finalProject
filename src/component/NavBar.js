import React from "react";

const NavBar = () => {
  return (
    <nav
      id="nav"
      class=" flex mb-8 bg-gradient-to-tl from-red-400 to-purple-500"
    >
      <img
        className="bg-white my-4 ml-8 mr-16 h-12 rounded-md"
        alt="solutelabs"
        src="data:image/svg+xml;base64,"
      />

      <h1 class="text-3xl  font-bold flex items-center text-gray-50">
        SoluteLabs
      </h1>

      <div class="container mx-auto py-4 flex justify-end items-center">
        <div class="flex">
          <a href="#section1">
            <div class="flex items-center mr-16">
              <span class="text-4xl text-gray-50 ml-2">&#128221;</span>
              <span class="text-gray-50 text-2xl font-bold">Form</span>
            </div>
          </a>
          <a href="#section2">
            <div class="flex items-center mr-16">
              <span class="text-4xl text-gray-50 ml-2">&#128214;</span>
              <span class="text-gray-50 text-2xl font-bold">Details</span>
            </div>
          </a>
        </div>
      </div>
    </nav>
  );
};

export default NavBar;
